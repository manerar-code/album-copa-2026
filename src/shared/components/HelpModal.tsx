          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {ITEMS.map(item => (
              <View key={item.label} style={styles.row}>
                <View
                  style={[styles.sample, { backgroundColor: item.color, borderColor: item.border }]}
                >
...
const styles = StyleSheet.create({
  ...
  sheet: {
    ...
    padding: spacing.lg,
    maxHeight: '80%',
    // flex layout para ScrollView funcionar dentro do maxHeight
    flexShrink: 1,
  },
  title: { ...typography.h2, color: '#0C1322', marginBottom: 4 },
  subtitle: { ...typography.body, color: '#646F88', marginBottom: 2 },
  cycle: {
    fontSize: 13,
    color: '#0C1322',
    fontWeight: '700',
  ...
  rowInfo: { flex: 1 },
  rowLabel: { fontSize: 15, fontWeight: '700', color: '#0C1322', marginBottom: 2 },
  rowDesc: { fontSize: 13, color: '#646F88', lineHeight: 18 },
  tutorialBtn: {
    borderWidth: 1,
    borderColor: '#0C1322',
  ...
  tutorialBtnText: { color: '#0C1322', fontWeight: '600', fontSize: 15 },
  closeBtn: {
  ...
  closeBtnText: { color: '#EEF2F8', fontWeight: '700', fontSize: 16 },
});
